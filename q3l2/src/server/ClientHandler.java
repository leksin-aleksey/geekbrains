...
//                }
                ...
                if (loginInfo.startsWith("-auth")) {
                    ...
                    if (maybeClient != null) {
                        if (!server.checkLogin(maybeClient.getName())) {
                            ...
                            sendMessage(String.format("%s is already logged in", maybeClient.getName()));
                        }
                    } else {
    ...
    public void readMessage() throws IOException {
        while (true) {
            ...
            //2.*Добавить в сетевой чат возможность смены ника.
                ...
                String[] splittedMessageInfo = message.split("\\s");
                String loginNew = splittedMessageInfo[1];
                if (server.changeLogin(client.getLogin(), loginNew)){
                    server.sendMessage(loginNew, "Your login has been changed successfully");
                }
            }
            else {
    ...
    private void setClient(AuthenticationService.Client client) {
        this.client = client;
    }
